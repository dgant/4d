...
class EditorControls extends THREE.EventDispatcher {
  constructor(object, domElement) {
    ...
    this.focus = function (target) {
      ...
      delta.set(0, 0, 1).applyQuaternion(object.quaternion).multiplyScalar(distance * 4);
      object.position.copy(center).add(delta);
      scope.dispatchEvent(changeEvent);
    };
    this.pan = function (delta) {
      var distance = object.position.distanceTo(center);
      delta.multiplyScalar(distance * scope.panSpeed).applyMatrix3(normalMatrix.getNormalMatrix(object.matrix));
    ...
    function onMouseMove(event) {
      ...
      if      (state === STATE.ROTATE){ scope.rotate(delta.set(- movementX, - movementY, 0)); }
      else if (state === STATE.ZOOM)  { scope.zoom(delta.set(0, 0, movementY)); }
      else if (state === STATE.PAN)   { scope.pan(delta.set(- movementX, movementY, 0)); }
      pointerOld.set(event.clientX, event.clientY);
    }
    function onMouseUp() {
      state = STATE.NONE;
